...
{
	progressiveKMean = cvCreateImage(cvGetSize(source),8,3); //Create an image container
...
RoadExtraction::ChiangKMean(IplImage* image,int profondeur)
{
   ...
   */
   ...
   // second cluster par ordre dÈcroissant d'occurences
...
// affiche la liste des couleurs utilisÈes dans un cluster
...
RoadExtraction::ManualKMean(int K, int b, int g, int r)
{
	...
	/*petit test
	CvMat *test = cvCreateMat(2, 2, CV_8UC1);

	SetMatElem(test, 0, 0, 0, 3);
	...
	*/
	...
	printf("barycentre 0 : %d %d %d\n",b,g,r);

	//Tirage des barycentres
	for(int i=1;i<K;i++)
	{
		bar_ch1 = rand() % 255 + 1;
		bar_ch2 = rand() % 255 + 1;
	...
	printf("test, barycentre: %f, %f, %f\n", barList->data.fl[3], barList->data.fl[4] ,barList->data.fl[5]);

	printf("1st loot ok\n");

	for(int it=0;it<10;it++)
	{
		//Appairage pixel-cluster
		for(int index=0;index<sample->rows/3;index++)
		{
			int baryID=0;
			int baryDist=abs(barList->data.fl[0]-sample->data.fl[index*3])+abs(barList->data.fl[1]-sample->data.fl[index*3+1])+abs(barList->data.fl[2]-sample->data.fl[index*3+2]);
			for(int j=1;j<barList->rows/3;j++)
			{
				int curDist = abs(barList->data.fl[3*j]-sample->data.fl[index*3])+abs(barList->data.fl[3*j+1]-sample->data.fl[index*3+1])+abs(barList->data.fl[3*j+2]-sample->data.fl[index*3+2]);
				if(curDist<baryDist)
				{
					baryDist = curDist;
					baryID = j;
		...
		for(int j=0;j<barList->rows/3;j++)
		{
			...
			for(int index=0;index<sample->rows/3;index++)
			{
				...
				{
					bSum+=(int)sample->data.fl[index*3];
					gSum+=(int)sample->data.fl[index*3+1];
					rSum+=(int)sample->data.fl[index*3+2];
					nbValue++;
				}
			}
	...
	//Affichage nouveaux barycentres
	for(int i=0;i<K;i++)
	{
		printf("Nouveaux barycentres %d : %f %f %f\n",i,barList->data.fl[3*i],barList->data.fl[3*i+1],barList->data.fl[3*i+2]);
	}
	...
	for (int it=0;it<K;it++)
	{
		...
		for(int i=0; i<dst_img->height; i++)
		{
			...
			{
				long int index = i*dst_img->width+j;
				if (cluster->data.i[index] == it)
				{
					CV_IMAGE_ELEM(dst_img,uchar, i, j*3) = sample->data.fl[index*3];
					CV_IMAGE_ELEM(dst_img,uchar, i, j*3+1) = sample->data.fl[index*3+1];
					CV_IMAGE_ELEM(dst_img,uchar, i, j*3+2) = sample->data.fl[index*3+2];
				}
			}
		}
